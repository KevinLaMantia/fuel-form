...
import { Search, Star, MapPin, DollarSign, Users, Award, Calendar } from "lucide-react"
...
export default function TrainersPage() {
  ...
  const filteredTrainers = trainers.filter((trainer) => {
    ...
    const matchesSpecialization =
      selectedSpecialization === "all" || trainer.specializations.includes(selectedSpecialization)
    const matchesLocation = selectedLocation === "all" || trainer.location.includes(selectedLocation)
    const matchesPrice =
      priceRange === "all" ||
      (priceRange === "under-75" && trainer.hourlyRate < 75) ||
      (priceRange === "75-90" && trainer.hourlyRate >= 75 && trainer.hourlyRate <= 90) ||
  ...
  const allSpecializations = Array.from(new Set(trainers.flatMap((t) => t.specializations)))
  const allLocations = Array.from(new Set(trainers.map((t) => t.location.split(", ")[1])))
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      <div className="w-full max-w-7xl xl:max-w-none 2xl:max-w-none mx-auto px-4 sm:px-6 lg:px-8 xl:px-12 2xl:px-16 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Find Your Perfect Trainer</h1>
          <p className="text-white/70">Connect with certified fitness professionals to reach your goals</p>
        </div>
        ...
        <div className="mb-8 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="flex items-center space-x-2">
              ...
              />
              <label htmlFor="verified" className="text-sm text-white cursor-pointer">
                Verified Only
              </label>
            </div>
          </div>
          ...
          {filteredTrainers.map((trainer) => (
            ...
            >
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-1 mt-1">
                        ...
                        <span className="text-white/60 text-sm">({trainer.reviews} reviews)</span>
                      </div>
                    </div>
                  </div>
                  <Badge
                    className={`${
                      trainer.availability === "Available" ? "bg-green-600 text-white" : "bg-orange-600 text-white"
                    }`}
                  >
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  <h4 className="text-white font-medium text-sm mb-2">Specializations</h4>
                  <div className="flex flex-wrap gap-1">
                    {trainer.specializations.map((spec) => (
                      <Badge key={spec} className="bg-purple-600/20 text-purple-300 border-purple-400/20 text-xs">
                        {spec}
                      </Badge>
                    ))}
                  </div>
                ...
                <div>
                  <h4 className="text-white font-medium text-sm mb-2">Certifications</h4>
                  <div className="flex flex-wrap gap-1">
                    {trainer.certifications.map((cert) => (
                      <Badge key={cert} className="bg-blue-600/20 text-blue-300 border-blue-400/20 text-xs">
                        {cert}
                      </Badge>
                    ))}
                  </div>
